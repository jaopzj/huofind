...
function ProfileHeader({
    ...
    const getTierInfo = (tier) => {
        ...
        if (tierLower.includes('minerador') || tierLower.includes('ouro') || tierLower.includes('gold')) {
            return { name: 'Minerador', color: 'from-yellow-400 to-yellow-600', bgColor: 'bg-yellow-100', textColor: 'text-yellow-700' };
        }
        if (tierLower.includes('escavador') || tierLower.includes('prata') || tierLower.includes('silver')) {
            return { name: 'Escavador', color: 'from-gray-300 to-gray-500', bgColor: 'bg-gray-100', textColor: 'text-gray-700' };
        }
        if (tierLower.includes('explorador') || tierLower.includes('bronze')) {
            return { name: 'Explorador', color: 'from-amber-400 to-amber-600', bgColor: 'bg-amber-100', textColor: 'text-amber-700' };
        }
        return { name: 'Convidado', color: 'from-orange-400 to-orange-600', bgColor: 'bg-orange-100', textColor: 'text-orange-700' };
    };
    ...
    return (
        <motion.div
            ...
            className="bg-white rounded-[2rem] p-6 md:p-8 shadow-xl border border-gray-100/50"
        >
                        ...
                        {isEditingName ? (
                            <div className="flex items-center gap-2">
                                <input
                                    ...
                                    autoFocus
                                    className="text-xl md:text-2xl font-black text-gray-900 bg-gray-50 border border-gray-200 rounded-xl px-3 py-1 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent"
